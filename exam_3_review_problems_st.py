...
def write_problem(prob_count: int, prob_key: str):
    st.subheader("Problem " + str(prob_count+1) + ": " + st.session_state.probstack[prob_key].title)
    st.button('New Version', key="new"+str(prob_key), on_click=new_problem, args=[prob_key])
    ...
    with st.expander("Show Solution"):
        st.write(st.session_state.probstack[prob_key].solution, unsafe_allow_html=True)
